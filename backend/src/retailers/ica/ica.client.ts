...
export async function searchIcaProducts(storeId: string, query: string): Promise<Product[]> {
    const url = new URL(`${ICA_BASE_URL}/${storeId}/api/webproductpagews/v6/product-pages/search`);
    ...
    const response = await fetchJson<IcaSearchResponse>(url.toString());

    const rawProducts = response.productGroups.flatMap((group) => group.decoratedProducts ?? []);
    ...
    return [...uniqueProducts.values()].map((product) => mapIcaProduct(product));
}
